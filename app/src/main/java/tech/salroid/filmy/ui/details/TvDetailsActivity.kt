...
class TvDetailsActivity : AppCompatActivity() {
    ...
    private fun collectUiStates() {
        lifecycleScope.launch {
            viewModel.uiStateTvDetails.collect {
                it?.let {
                    showTvDetails(it)
    ...
    private fun setupListeners() {
        ...
        binding.backdrop.setOnClickListener {
            if (bannerForFullScreen != null) {
                val intent = Intent(this@TvDetailsActivity, FullBannerActivity::class.java)
    ...
    private fun getDataFromIntent(intent: Intent?) {
        intent?.let {
            ...
            val movieType = it.getSerializableExtra(MOVIE_TYPE) as? Movie.MovieType
            type = movieType?.ordinal ?: 0
            tvId = it.getStringExtra(MOVIE_ID)
    ...
    private fun getTvDetails() {
        if (!this::tvDetails.isInitialized) {
            ...
            viewModel.getTvDetails(tvId, type)
        }
    }

    private fun showCastFragment() {
        val castFragment =
            CastCrewFragment.newInstance(
                null,
                movieTitle,
                CastCrewFragment.CastCrewType.CAST,
                isTv = true
            )
    ...
    private fun showCrewFragment() {
        val crewFragment =
            CastCrewFragment.newInstance(
                null,
                movieTitle,
                CastCrewFragment.CastCrewType.CREW,
                isTv = true
            )
        supportFragmentManager
            .beginTransaction()
    ...
    private fun showSuggestionFragment(
        ...
        supportFragmentManager
            ...
            .replace(
                containerId,
                SimilarRecommendationFragment.newInstance(
                    null,
                    movieTitle,
                    suggestionType,
                    isTv = true
    ...
    private fun showTvDetails(movie: TvDetails) {
        this.tvDetails = movie
        ...
        binding.detailTitle.text = movie.name
        ...
        val hours = movie.episodeRunTime.firstOrNull()?.div(60) ?: 0
        val mins = movie.episodeRunTime.firstOrNull()?.rem(60) ?: 0
        ...
        binding.viewExtraInfo.released.text = buildSpannedString {
            if(movie.episodeRunTime.firstOrNull() != null) {
                append("\u00A0") // nbsp
                append("\u00A0") // nbsp
                append("\u2022  ")
            }
            append(movie.firstAirDate?.toReadableDate())
        }
        ...
        // Get Reviews
        viewModel.getTvReviews(tvId)

        // Get Watch Providers
        viewModel.getWatchProvidersTv(tvId)

        // Get Cast, Crew and Similar Movies
        movieIdFinal?.let {
            castViewModel.getCastAndCrewTv(it)
            similarViewModel.getSimilarTv(it)
            similarViewModel.getRecommendationTv(it)
        }
    ...
    private fun setRating() {
        ...
        // TMDB User Score
        val userScore = tvDetails.voteAverage?.times(10)?.toInt() ?: 0
        ...
        binding.viewRatings.tmdbRatingBy.text = buildSpannedString {
            append(" \u2022 ${tvDetails.voteCount} Ratings")
        }
        ...
        //if (TvDetails.voteAverage == 0.0) binding.viewRatings.layoutTmdb.visibility = View.GONE else {
    ...
    private fun setSingleReviewData(
        ...
        with(reviewItemLayoutBinding) {
            ...
            Glide.with(this@TvDetailsActivity)
                .load(review.authorDetails?.getAvatarUrl(this@TvDetailsActivity))
    ...
    private fun addWatchlist() {
        /* TvDetails.watchlist = true
         TvDetails.type = type
         viewModel.updateTvDetailsInDb(TvDetails, WATCHLIST, false)*/
    }

    private fun addFavorite() {
        /* TvDetails.favorite = true
         TvDetails.type = type
         viewModel.updateTvDetailsInDb(TvDetails, FAVOURITES, false)*/
    }

    private fun removeWatchlist() {
        /* TvDetails.watchlist = false
         TvDetails.type = type
         viewModel.updateTvDetailsInDb(TvDetails, WATCHLIST, true)*/
    }

    private fun removeFavorite() {
        /*  TvDetails.favorite = false
          TvDetails.type = type
          viewModel.updateTvDetailsInDb(TvDetails, FAVOURITES, true)*/
    }

    private fun openCustomTabIntent(url: String, color: Int) {
        val builder = CustomTabsIntent.Builder()
        builder.setToolbarColor(ContextCompat.getColor(this@TvDetailsActivity, color))
